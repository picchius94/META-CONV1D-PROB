## Path PLanner for planning_experiment_quantitative_v3.py
...
class A_star:
    def __init__(self, params):
        ...
        self.timeout = params["MAX_TIMEOUT"]
        
        # Memory shots
        self.all_methods = params["WHICH_METHOD"]
        self.params_model = {}
        self.model = {}
        self.memory_list = {}
        self.memory = {}
    ...
    def set_models(self, params_model):
        import tensorflow as tf
        flag_conv1D = False
        flag_pr = False
        
        for method in self.all_methods:
            self.params_model[method] = params_model[method]
        self.method = self.params["PLANNING_METHOD"]
    ...
    def normalize_input_features(self, X, method = None):
        if method is None:
            method = self.method
        for i in range(X.shape[-1]):
            X[...,i] = (X[...,i]-self.params_model[method]["INPUT_FEATURES_val1"][i])/self.params_model[method]["INPUT_FEATURES_val2"][i]
        return X
    def normalize_input_features_separate(self, X, sep_model, method = None):
        if method is None:
            method = self.method
        model = self.params_model[method]["sep_models"][sep_model]
        for i in range(X.shape[-1]):
            X[...,i] = (X[...,i]-model["INPUT_FEATURES_val1"][i])/model["INPUT_FEATURES_val2"][i]
        return X
        
    def normalize_extra_info(self, X, method = None):
        if method is None:
            method = self.method
        for i in range(X.shape[-1]):
            X[...,i] = (X[...,i]-self.params_model[method]["SHOTS_EXTRA_INFO_val1"][i])/self.params_model[method]["SHOTS_EXTRA_INFO_val2"][i]
        return X
        
    def normalize_energy(self, energy, method = None):
        if method is None:
            method = self.method
        try:
            energy += self.params_model[method]["EPS"]
        except:
            pass
        energy = (energy-self.params_model[method]["energy_val1"])/self.params_model[method]["energy_val2"]
        if 'shift' in self.params_model[method]["ENERGY_NORM_TYPE"]:
            energy += self.params_model[method]["energy_val1"]/self.params_model[method]["energy_val2"]
        return energy

    def denormalize_energy_tot(self, energy, method = None):
        if method is None:
            method = self.method
        if 'shift' in self.params_model[method]["ENERGY_NORM_TYPE"]:
            energy -= self.params_model[method]["energy_val1"]/self.params_model[method]["energy_val2"]*self.params_model[method]["LENGTH_SEQUENCE"]
        energy = energy*self.params_model[method]["energy_val2"] + self.params_model[method]["energy_val1"]*self.params_model[method]["LENGTH_SEQUENCE"]
        try:
            energy -= self.params_model[method]["EPS"]*self.params_model[method]["LENGTH_SEQUENCE"]
        except:
            pass
        return energy

    def denormalize_energy_tot_sep_model(self, energy, sep_model, method = None):
        if method is None:
            method = self.method
        model = self.params_model[method]["sep_models"][sep_model]
        if 'shift' in self.params_model[method]["ENERGY_NORM_TYPE"]:
            energy -= model["energy_val1"]/model["energy_val2"]*self.params_model[method]["LENGTH_SEQUENCE"]
        energy = energy*model["energy_val2"] + model["energy_val1"]*self.params_model[method]["LENGTH_SEQUENCE"]
        try:
            energy -= self.params_model[method]["EPS"]*self.params_model[method]["LENGTH_SEQUENCE"]
        except:
            pass
    ...
    def wheel_trace(self, i, pi):
        ...
        try:
            ...
            idx_x = np.clip(idx_x, 0, self.DEM_size_x-1)
            idx_y = np.clip(idx_y, 0, self.DEM_size_y-1)
            Z_map = self.Z[idx_x, idx_y]
    ...
    def segments_stats(self, data_run, ref_traj):
        ...
        while data_id+1 < len_stats:
            ...
            while data_id < len_stats:
                ...
                meas_acc_long.append(stats["FWD_Acc"].values[data_id])
                meas_acc_lat.append(stats["Lat_Acc"].values[data_id])
                meas_acc_v.append(stats["V_Acc"].values[data_id])
            ...
            stat_data.append({"energy": energy_segment,
                              ...
                              "mean_acc_long": np.mean(np.abs(meas_acc_long)), "mean_acc_lat": np.mean(np.abs(meas_acc_lat)), "mean_acc_v": np.mean(np.abs(meas_acc_v)),
                              "mean_pitch_meas": np.mean(meas_pitches), "var_pitch_meas": np.var(meas_pitches),
                              "mean_roll_meas": np.mean(np.abs(meas_rolls)), "var_roll_meas": np.var(meas_rolls),
    ...
    def estimate_pitch_roll(self, xi_ref, yi_ref, yawi_ref, remove_unpickable=True):
        ...
        for segment in range(self.n_segments):
            ...
            # res = np.array(ray.get([self.starting_pose_remote.remote(self,p) for p in zip(xi_ref[start_id:end_id],yi_ref[start_id:end_id],yawi_ref[start_id:end_id])]))
            # est_pitches = res[:,1]
            # est_rolls = res[:,0]
    ...
    def estimate_wheel_trace(self, xi_ref, yi_ref, yawi_ref, remove_unpickable = True):
    ...
    def advance_state(self, ida, state):
        ...
        next_state = (round(xi_ref[-1],2),round(yi_ref[-1],2),round(yawi_ref[-1],2))
        # Check Safety
    ...
    def create_shots_batch(self, n_shots = None, energy_method = None):
        ...
        if n_shots:
            ...
            if 'meta' in self.method:
                xe_shots = self.normalize_energy(xe_shots, energy_method)
    ...
    def estimate_energy(self, outputs, remove_unpickable = True):
        ...
        if self.n_shots:
            ...
            for method in self.all_methods:
                if "prob" in method:
                    distr = self.model[method]([self.XG_SHOTS[:bsize], self.XE_SHOTS_PROB[method][:bsize], XG_META])[self.n_shots-1]
                    means[method] = distr.mean().numpy()
                    stddevs[method] = distr.stddev().numpy()
                    modes[method] = distr.mode().numpy()
                    if self.params_model[method]["LIST_PPF_MIN"] is not None and self.params_model[method]["LIST_PPF_MAX"] is not None:
                        ppfs_max[method] = {}
                        ppfs_min[method] = {}
                        if "gamma" in method:
                            alphas = (means[method]/stddevs[method])**2
                            bethas = means[method]/stddevs[method]**2
                            for pmin, pmax in zip(self.params_model[method]["LIST_PPF_MIN"],self.params_model[method]["LIST_PPF_MAX"]):
                                ppfs_max[method][pmax] = gamma.ppf(pmax, alphas, scale = 1/bethas)
                                ppfs_min[method][pmin] = gamma.ppf(pmin, alphas, scale = 1/bethas)
                        elif "lognorm" in method:
                            sigs = np.sqrt(np.log((stddevs[method]/means[method])**2+1))
                            mus = np.log(means[method])-sigs**2/2
                            for pmin, pmax in zip(self.params_model[method]["LIST_PPF_MIN"],self.params_model[method]["LIST_PPF_MAX"]):
                                ppfs_max[method][pmax] = lognorm.ppf(pmax, sigs, scale = np.exp(mus))
                                ppfs_min[method][pmin] = lognorm.ppf(pmin, sigs, scale = np.exp(mus))
                    
                    means[method] = np.clip(self.denormalize_energy_tot(means[method], method),0.0,None)
                    stddevs[method] = stddevs[method]*self.params_model[method]["energy_val2"]
                    modes[method] = np.clip(self.denormalize_energy_tot(modes[method], method),0.0,None)
                    if self.params_model[method]["LIST_PPF_MIN"] is not None and self.params_model[method]["LIST_PPF_MAX"] is not None:
                        for pmin, pmax in zip(self.params_model[method]["LIST_PPF_MIN"],self.params_model[method]["LIST_PPF_MAX"]):
                            ppfs_max[method][pmax] = np.clip(self.denormalize_energy_tot(ppfs_max[method][pmax], method),0.0,None)
                            ppfs_min[method][pmin] = np.clip(self.denormalize_energy_tot(ppfs_min[method][pmin], method),0.0,None)
                            
        else:
            energies = np.zeros((bsize,))
            for method in self.all_methods:
                if "prob" in method:
                    means[method] = np.zeros((bsize,))
                    stddevs[method] = np.zeros((bsize,))
                    modes[method] = np.zeros((bsize,))
                    if self.params_model[method]["LIST_PPF_MIN"] is not None and self.params_model[method]["LIST_PPF_MAX"] is not None:
                        ppfs_max[method] = {}
                        ppfs_min[method] = {}
                        for pmin, pmax in zip(self.params_model[method]["LIST_PPF_MIN"],self.params_model[method]["LIST_PPF_MAX"]):
                            ppfs_max[method][pmax] = np.zeros((bsize,))
                            ppfs_min[method][pmin] = np.zeros((bsize,))
        ...
        for i,output in enumerate(outputs):
            if output["safe"]:
                outputs[i]["cost"] = energies[id_e].item()
                for method in self.all_methods:
                    if "prob" in method:
                        outputs[i]["mode_{}".format(method)] = modes[method][id_e].item()
                        outputs[i]["mean_{}".format(method)] = means[method][id_e].item()
                        outputs[i]["stddev_{}".format(method)] = stddevs[method][id_e].item()
                        if self.params_model[method]["LIST_PPF_MIN"] is not None and self.params_model[method]["LIST_PPF_MAX"] is not None:
                            for pmin, pmax in zip(self.params_model[method]["LIST_PPF_MIN"],self.params_model[method]["LIST_PPF_MAX"]):
                                outputs[i]["ppf_max_{}_{}".format(pmax,method)] = ppfs_max[method][pmax][id_e].item()
                                outputs[i]["ppf_min_{}_{}".format(pmin,method)] = ppfs_min[method][pmin][id_e].item()   
    ...
    def neighbours(self,state):
        ...
        if self.optimization_criteria == "energy":
            heuristics = [self.heuristic(output["next_state"]) if output["safe"] and not output["goal"] else None for output in outputs]
    ...
    def search(self, start, end, optimization_criteria = 'energy', method = "meta", straight_flag=False):
        ...
        if self.optimization_criteria == "energy":
            ...
            for method in self.all_methods:
                if "prob" in method:
                    _, self.XE_SHOTS_PROB[method] = self.create_shots_batch(energy_method = method)
